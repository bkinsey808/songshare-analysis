...
.PHONY: help install install-mamba songshare-analyze-cpu poetry-env test lint format clean

help:
	@echo "Available commands: install, install-mamba, songshare-analyze-cpu (alias: essentia-env; creates songshare-analyze-cpu), poetry-env, test, lint, format, clean"
...
# Create or update the 'songshare-analyze-cpu' conda/mamba environment (canonical target)
songshare-analyze-cpu:
...
test:
	@if command -v poetry >/dev/null 2>&1; then \
		...
		if command -v mamba >/dev/null 2>&1 || command -v conda >/dev/null 2>&1; then \
			...
			echo "Run tests inside your CPU Conda/Mamba env (e.g., make songshare-analyze-cpu && mamba activate songshare-analyze-cpu && pip install -e . && pytest)"; \
		fi; \
	fi
...
lint:
	@if command -v poetry >/dev/null 2>&1; then \
		...
		# Try to set up the 'songshare-analyze-cpu' env and run lint inside it
		if command -v mamba >/dev/null 2>&1 || command -v conda >/dev/null 2>&1; then \
			./scripts/setup-analyze.sh && mamba run -n songshare-analyze-cpu make lint; \
		else \
				echo "Poetry not found. To run linting use the 'songshare-analyze-cpu' conda env: make songshare-analyze-cpu && mamba run -n songshare-analyze-cpu pip install poetry && mamba run -n songshare-analyze-cpu poetry install && make lint"; \
	fi
...
typecheck:
	@if command -v poetry >/dev/null 2>&1; then \
		...
		echo "Poetry not found. To run typecheck use the 'songshare-analyze-cpu' conda env: make songshare-analyze-cpu && conda activate songshare-analyze-cpu && pip install -e . && make typecheck"; \
	fi
...
format:
	@if command -v poetry >/dev/null 2>&1; then \
		...
		echo "Poetry not found. To format code use the 'songshare-analyze-cpu' conda env: make songshare-analyze-cpu && conda activate songshare-analyze-cpu && pip install -e . && make format"; \
	fi
...
analyze-test: songshare-analyze-cpu
	@echo "Running analyze tests in conda env 'songshare-analyze-cpu'"
...
# Convenience: install mambaforge if needed, create env, and run analyze tests
analyze-setup-test:
